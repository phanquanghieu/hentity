    ...
    show ? (
      <div className='fixed top-0 left-0 right-0 bottom-0 z-50 bg-slate-700/40'>
        <div className='h-full p-5 overflow-auto flex justify-center'>
          <div
            className={twMerge(
              'relative w-[50rem] h-fit my-0 bg-white rounded-md',
              className
            )}
          >
            <div
            ...
            {header && (
              <div className='py-4 px-5 rounded-t-md border-b border-slate-200 bg-white text-2xl text-slate-700 font-medium'>
                {header}
              </div>
            )}
            {children}
